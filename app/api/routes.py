...
async def get_live_feeds():
    return {"channels": await asyncio.to_thread(recordings.get_live_channels)}

@router.get("/dates")
async def get_dates(channel: int = Query(None, ge=1, description="Channel number (1-based)")):
    """
    ...
    return {"dates": await asyncio.to_thread(recordings.get_available_dates, channel)}
...
async def get_recordings(
    ...
    return {"recordings": await asyncio.to_thread(recordings.get_recordings_for_date, ch, date)}

@router.delete("/recording")
async def delete_recording(path: str = Query(..., description="Relative path to the recording file")):
    ...
    mtime = await asyncio.to_thread(os.path.getmtime, abs_path)
    ...
    try:
        ...
        if parent != os.path.abspath(config.settings.record_dir) and await asyncio.to_thread(os.path.isdir, parent) and not await asyncio.to_thread(os.listdir, parent):
            await asyncio.to_thread(os.rmdir, parent)
...
async def restart_youtube_stream():
